...
{
    public class KolibriTVShowSearchController
    {
        ...
        public KolibriTVShowSearchController(UserSettings userSettings, LiteDBController liteDB = null, TMDBController tmdb = null, OMDBController omdb = null)
        {
        ...
        public async Task<List<KolibriTVShow>> SearchForSeriesAsync(DirectoryInfo source)
        {
            ...
            var directoryArr = source.GetDirectories("*.*", SearchOption.AllDirectories);
            if (directoryArr.Count() == 0)
            {
                directoryArr = new List<DirectoryInfo>() { source}.ToArray();
               
            }
            foreach (DirectoryInfo dir in directoryArr)
            {
                fList = Kolibri.net.Common.Utilities.FileUtilities.GetFiles(dir, MovieUtilites.MoviesCommonFileExt(true), SearchOption.AllDirectories);
                ...
                if (item != null)
                {
                    ...
                    if (show != null) { show.Item.TomatoUrl = dir.FullName; list.Add(show); continue; }
    ...
    var whatNow = await _TMDB.FindById(imdbid);
                    ...
                    if (item != null)
                    {
                        ...
                        if (show != null) { show.Item.TomatoUrl = dir.FullName; list.Add(show); continue; } 
                            ...
                            SetStatusLabelText($"{seriesTitle} was not found", "NOTFOUND");
                        }
                ...
                seriesTitle = MovieUtilites.GetMovieTitle(dir.FullName);
                var tmpList = Kolibri.net.Common.Utilities.FileUtilities.GetFiles(dir, MovieUtilites.MoviesCommonFileExt(true), SearchOption.AllDirectories);
                if (tmpList.Count() < 1) { continue; }
                else fList.AddRange(tmpList);
        ...
        private async Task<Item> GetItemByIdOrNameAsync(string seriesName, int year)
        {
            Item item = null;
            try
            {
                ...
                if (item == null)
                {
                    ...
                    if (seriesSearchItemList.Count >= 1)
                    {
                        ...
                        {
                            item = await _OMDB.GetMovieByIMDBidAsync(seriesSearchItem.ImdbId, insert: true);
                            if (item != null) { await _liteDB.UpsertAsync(item); }
        ...
        private async Task<Item> GetItemAsync(FileInfo file, int year, string title)
        {
            Item movie = null;
                ...
                if (movie == null)
                {
                    try
                    {
                        ...
                        if (tLibList != null && tLibList.Count == 1)
                        {
                            ...
                            {
                                movie = await _TMDB.GetMovieAsync(tLibList[0]);
                            }
                        }
                        else
                        {
        ...
        public async Task<KolibriTVShow> GetShowByIdAsync(string imdbid)
        {
                        ...
                        foreach (var ep in kses.Episodes)
                        {
                           ...
                           Episode kep = _liteDB.GetEpisode(ep.ImdbId);
                            if (kep == null)
                            {
                                ...
                                if (omdb != null)
                                {
                                    kep = JsonConvert.DeserializeObject<Episode>(omdb.JsonSerializeObject());
                                }
                                else
                                {
                                    kep = JsonConvert.DeserializeObject<Episode>(ep.JsonSerializeObject());
                                }
                                kep.EpisodeNumber = ep.Episode;
                                    kep.SeasonNumber = kses.SeasonNumber;
            ...
            //return Task.Delay(2).ContinueWith(t => tv);
            return tv;
        }
        ...
        */
        private async Task< List<KolibriTVShow>> GetKolibriTVShows(List<Episode> epList, string showName = null)
        {
                ...
                try
                {
                    ...
                    if (show.Item == null)
                    {
                        ...
                        if (show.Item == null && !string.IsNullOrEmpty(omdbEpisode.ImdbId))
                        {
                            ...
                            if (show.Item != null) await _liteDB.UpsertAsync(show.Item);
                        }
                    }
        ...
        private async Task<DataTable> SearchForSeriesEpisodes(List<string> titleList)
        {
            ...
            foreach (var title in titleList)
            {
                ...
                try
                {
                    ...
                    var item = await GetItemByIdOrNameAsync(currentItem, 0);
                    if (item != null)
                    {
        ...
        private async Task< DataTable> SearchForSeriesEpisodesList(List<string> titleList)
        {
            string currentItem = "Uten navn";
            var totEpisodes = new List<Episode>();
            foreach (var title in titleList)
                ...
                try
                {
                    ...
                    using (var e = await Task.Run(() => Directory.EnumerateFiles(dir.FullName, filter, new EnumerationOptions() { RecurseSubdirectories = true }).GetEnumerator()))
                    {
                        while (await Task.Run(() => e.MoveNext()))
                        {
                            ...
                            Item movie = await GetItemAsync(file, year, title);
                            if (movie != null)
                            {
