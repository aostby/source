...
All files (*.*)|*.*";
            ...
            dic.Add("Excel", "xml");
            // sfd.Filter = Utilities.FileUtilities.GetFileDialogFilter(dic);
            // sfd.FilterIndex = MetaDataController.GetFilterIndexFromRegistry(m_controller.Systemnr);
            ...
            try
            {
                ...
                if (sfd.FilterIndex != 4 & sfd.FilterIndex != 7 & sfd.FilterIndex != 9)
                {
                    ...
                    //ds = m_dbConnection.GetDataSet(sql);//GenerellController.DataSetSelect(sql, m_dbConnection);
                    //if (ds == null) { toolStripStatusLabel1.Text = "Query returned no result, aborting save"; return; }
                    //table = ds.Tables[0];
                ...
                switch (sfd.FilterIndex)
                {
                    ...
                    //http://dcx.sybase.com/index.html#1201/en/dbusage/ug-sqlxml-a--s-3475372a.html
                    //  string sql = string.Format(@"SELECT XMLELEMENT( NAME {0},( {1} FOR XML AUTO, ELEMENTS) )", textBoxMethodName.Text, sql);
                    // result = m_dbConnection.ExecuteScalar(sql, new object[0]).ToString(); break;
                    case 5:
                        ...
                        // case 6: CLRClassGeneratorController clrContr = new CLRClassGeneratorController(); result = clrContr.DataTableToEntityClass("CLRExtEnv.Entities", table, m_dbConnection.GetData(MetaDataController.DescribeQuery(sql), new object[0])); break;
                        // case 7: UnloadController.UnloadTable(new FileInfo(sfd.FileName), sql, new SAClient(m_dbConnection.DbConnectionString)).ToString();
                        //     result = string.Empty;
                        //  case 8: result = MetaDataController.SQLFormatter(sql); ; break;
                        //   case 6:
                        //  case 7:
                        //  case 8:result = data.ToString();
                        break;
        ...
        public static bool SlettBrukerPDFFiler(string directory)
        {
            ...
            try
            {
                ...
                if (!directory.Equals(directoryInfoApplikasjon.ToString()))
                {
                    ...
                    if (SystemInfo.GetLoggedOnUser.Equals("aostby"))
                    {
