					...
					<td>
						...
						<a>
							<xsl:attribute name="href">
								<xsl:value-of select="concat('https://www.brewersfriend.com/fermentables/', ./NAME/text())" />
							</xsl:attribute>
							<xsl:value-of select="./NAME"/>
						</a>
			...
			<xsl:for-each select="//HOP">
				<tr style="font-size: 12px; background: #ffffff;">
					...
					<td> 
						...
						<a>
							<xsl:attribute name="href">
								<xsl:value-of select="concat('https://www.brewersfriend.com/hops/', ./NAME/text())" />
							</xsl:attribute>
							<xsl:value-of select="./NAME"/>
						</a>
			...
			<table cellspacing="0" cellpadding="4" style="margin-bottom: 10px; background-color: #EEE; border: #D2D2D2 1px solid; width: 100%; border-spacing: 0px 0px; padding: 5px; text-align: left;">
				<tr style="font-size: 12px;">
					<td style="font-size: 15px;">
						...
						<a>
						<xsl:attribute name="href">
							<xsl:value-of select="concat('https://www.brewersfriend.com/yeasts/', ./NAME/text())" />
						</xsl:attribute>
						<xsl:value-of select="./NAME"/>
					</a>
					</td>
...
<!-- Brukes av bla NOTESS-->
	...
	<!-- Brukes svært mange steder. Dersom en trenger omgjøring til andre verdier, bør det være egne metoder-->
	<xsl:template name="round">
		<xsl:param name="value"/>   
	 	<xsl:value-of select='format-number( round(100*$value) div 100 ,"##0.00" )' />   
	</xsl:template>  
	
	<!--https://www.brewersfriend.com/fermentables/german-pilsner/--> 
</xsl:stylesheet>
