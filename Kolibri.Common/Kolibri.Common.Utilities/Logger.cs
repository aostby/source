...
namespace Kolibri.Common.Utilities
{
    public class Logger
    {
         ...
         */
         ...
         // 8   Filsti			
            ...
            try
            {
                ...
                tekstArr[0] = SystemInfo.GetLoggedOnUser; //bruker
        ...
        public static void WriteAllLogs(Kolibri.Data.Connection.DbConnection dbConnection)
        {
            WriteEventLog();
        ...
        private static void WriteSystemLog(Kolibri.Data.Connection.DbConnection  dbConnection)
        {
            if (dbConnection != null && m_loggListe != null && m_loggListe.Count > 0)
            {
                for (int i = 0; i < m_loggListe.Count; i++)
                {
                    string sql = String.Format("insert into SR022SystemLogg("
                    + "Bruker"
                    + ",LoggType"
                    + ",Tidspunkt"
                    + ",LoggTekst"
                    ...
                    + ",Filsti"
                        ...
                        + ") values('{0}')", String.Join("','", ((string[])m_loggListe[i])));
                    try
                    {
                        sql = sql.Replace(",''", ",NULL");
                        if (dbConnection.SqlExecute(sql) >= 1)
                            dbConnection.Commit();
                        else
