...
namespace Kolibri.Data.Connection
{
    ...
    {
        private static string ODBC_LOC_IN_REGISTRY = "SOFTWARE\\ODBC\\";
        private static string ODBC_INI_LOC_IN_REGISTRY =
            ODBC_LOC_IN_REGISTRY + "ODBC.INI\\";

        private static string DSN_LOC_IN_REGISTRY =
            ODBC_INI_LOC_IN_REGISTRY + "ODBC Data Sources\\";

        private static string ODBCINST_INI_LOC_IN_REGISTRY =
            ODBC_LOC_IN_REGISTRY + "ODBCINST.INI\\";

        private static string ODBC_DRIVERS_LOC_IN_REGISTRY =
        ...
        private static void set64BitConstants(bool set64bit)
        {
            if (set64bit)
                ODBC_LOC_IN_REGISTRY = "SOFTWARE\\Wow6432Node\\ODBC\\";
            else
                ODBC_LOC_IN_REGISTRY = "SOFTWARE\\ODBC\\";
            ODBC_INI_LOC_IN_REGISTRY = ODBC_LOC_IN_REGISTRY + "ODBC.INI\\";

            DSN_LOC_IN_REGISTRY = ODBC_INI_LOC_IN_REGISTRY + "ODBC Data Sources\\";

            ODBCINST_INI_LOC_IN_REGISTRY = ODBC_LOC_IN_REGISTRY + "ODBCINST.INI\\";

            ODBC_DRIVERS_LOC_IN_REGISTRY = ODBCINST_INI_LOC_IN_REGISTRY + "ODBC Drivers\\";
        }
        ...
        {
            ODBCDSN[] temp = null;
            ODBCDSN[] ret = GetDSNList(Registry.LocalMachine);
            if (is64Bit())
            {
                set64BitConstants(true);

                temp = GetDSNList(Registry.LocalMachine);

            }
            if (temp != null && temp.Length > 0)
            {
                ArrayList temparrl = new ArrayList(temp);
                temparrl.AddRange(ret);
                ret = (ODBCDSN[])temparrl.ToArray(typeof(ODBCDSN));             
            }
            return ret;
        }
        ...
        private static ODBCDSN[] GetDSNList(RegistryKey baseKey)
        {
            ...
            // RegOpenKeyEx(key, s, 0, KEY_READ | KEY_WOW64_64KEY, &key);
        ...
        public static RegistryKey OpenComplexSubKey(RegistryKey baseKey, string complexKeyStr, bool writable)
        {
            ...
            //RegistryKey fianlKey = (Registry.LocalMachine).OpenSubKey("Software\ODBC");
        }
    }
}
