...
namespace Kolibri.Common.MovieAPI.Controller
{
    public class ImagePosterDBController
    {
        ...
        public ImagePosterDBController(bool exclusiveAccess = false, bool readOnly = true)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(Properties.Settings.Default.LastImageCacheConnectionString))
                    if (File.Exists(Properties.Settings.Default.LastImageCacheConnectionString))
                    {
                        ConnectionString.Filename = Properties.Settings.Default.LastImageCacheConnectionString;
                    }
            }
            catch (Exception) { }
            ...
            try
            {
                Properties.Settings.Default.LastImageCacheConnectionString = ConnectionString.Filename;
        ...
        public bool InsertImage(ImagePoster imagePoster)
        {
            try
            {
        ...
        public ImagePoster FindImage(string imdbId)
        {
           ...
           //     .Find(x => x.ImdbId == imdbId).FirstOrDefault();
                ...
                if (imgbase != null)
                {
                    ret = new ImagePoster(imgbase.ImdbId, Kolibri.Common.Utilities.ImageUtilities.Base64ToImage(imgbase.Base64) as Bitmap);
                }
                return ret;
            }
            catch (Exception ex)
