...
namespace SortPics.Controller
{
    public class SeriesCache
    {
        ...
        public SeriesCache(DirectoryInfo cachePath)
        {
            ...
            try
            {
                ...
                if (_seasonEpDicPath.Exists)
                    _seasonEpDic = new System.Web.Script.Serialization.JavaScriptSerializer().Deserialize<Dictionary<string, SeasonEpisode>>(File.ReadAllText(_seasonEpDicPath.FullName));
            }
            catch (Exception) { }
            ...
            try
            {
                ...
                if (_seasonDicPath.Exists)
                    _seasonDic = new System.Web.Script.Serialization.JavaScriptSerializer().Deserialize<Dictionary<string, Season>>(File.ReadAllText(_seasonDicPath.FullName));
            }
            catch (Exception) { } 
        }
        ...
        internal List<Season> Get(string showTitle, int season )
        {
        ...
        internal   SeasonEpisode  Get(string seriesTitle, int season, int episode)
        {
            try
            {
  string id = $"{seriesTitle}{season}{episode}";
        ...
        public bool Save()
        {
            ...
            try
            {
                if (_seasonEpDic.Count() > 0)
                    File.WriteAllText(_seasonEpDicPath.FullName, new System.Web.Script.Serialization.JavaScriptSerializer().Serialize(_seasonEpDic));

                if (_seasonDic.Count() > 0)
                    File.WriteAllText(_seasonDicPath.FullName, new System.Web.Script.Serialization.JavaScriptSerializer().Serialize(_seasonDic)); 
            }
            catch (Exception)
            {
                ret = false;
        ...
        internal SearchTv SearchTvCacheGet(string showTitle)
        {
